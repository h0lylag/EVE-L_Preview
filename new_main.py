...
class ThumbnailsTab(QWidget):
    def __init__(self):
        ...
        self.en_cb = QCheckBox("Enable Borders")
        self.en_cb.setChecked(config["settings"]["enable_borders"])
        self.en_cb.stateChanged.connect(self.toggle)

        self.lab_act = QLabel("Active Client Border Color:")
        self.inp_act = QLineEdit(config["settings"]["active_border_color"])
        self.lab_in = QLabel("Inactive Client Border Color:")
        self.inp_in = QLineEdit(config["settings"]["inactive_border_color"])

        lay.addWidget(self.en_cb)
        lay.addWidget(self.lab_act); lay.addWidget(self.inp_act)
        lay.addWidget(self.lab_in); lay.addWidget(self.inp_in)

        self.inp_act.editingFinished.connect(self.set_act)
        self.inp_in.editingFinished.connect(self.set_in)

    def toggle(self, st):
        config["settings"]["enable_borders"] = bool(st)
        save_config(config)

    def set_act(self):
        c = self.inp_act.text()
        if self.valid_hex(c):
            config["settings"]["active_border_color"] = c
            save_config(config)

    def set_in(self):
        c = self.inp_in.text()
        if self.valid_hex(c):
            config["settings"]["inactive_border_color"] = c
            save_config(config)

    @staticmethod
    def valid_hex(c):
        return c.startswith("#") and len(c) == 7 and all(ch in "0123456789abcdefABCDEF" for ch in c[1:])
...
class GeneralTab(QWidget):
    def __init__(self):
        super().__init__()
        lay = QVBoxLayout(self)
        lay.addWidget(QCheckBox("General option 1"))
        lay.addWidget(QCheckBox("General option 2"))
...
class SettingsTab(QWidget):
    def __init__(self):
        super().__init__()
        lay = QVBoxLayout(self)
        lay.addWidget(QCheckBox("Settings option 1"))
        lay.addWidget(QCheckBox("Settings option 2"))
...
class ProfilesTab(QWidget):
    def __init__(self):
        super().__init__()
        lay = QVBoxLayout(self)
        lay.addWidget(QCheckBox("Profiles option 1"))
        lay.addWidget(QCheckBox("Profiles option 2"))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tray.activated.connect(lambda r: (self.show(), self.raise_(), self.activateWindow())
        ...
        self.tabs.addTab(ThumbnailsTab(), "Thumbnails")
    ...
    def load_pos(self):
        x, y = config["settings"].get("application_position", [100, 100])
        self.move(x, y)
